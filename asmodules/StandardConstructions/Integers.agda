...
open import StandardConstructions.Naturals
    using ( Nat; zero; suc; add; mul; l-add-zero; r-add-zero; add-comm; suc-skip-add; add-ass; mul-comm; r-one-neutral; r-zero-absorbs; r-add-one-suc;
            Pos; p1; one; pos-add; pos-mul; pos-mul3; pos-mul3-lass; pos-add-ass; pos-add-comm; posmul-ass; posmul-comm; pos-mul-lunital; pos-mul-runital; pos-mul-ldist )
...
symmint-succ : SymmInt -> SymmInt
symmint-succ (pos (p1 x)) = pos ( p1 ( suc x ) )
symmint-succ szero = pos ( p1 zero )
symmint-succ (neg (p1 zero)) = szero
symmint-succ (neg (p1 (suc x))) = neg (p1 x )

symmint-succ-def-pos : ( x : Nat ) -> ( definition-equal ( pos ( p1 ( suc x ) ) ) ( symmint-succ ( pos ( p1 x ) ) ) )
symmint-succ-def-pos x = 🐓🥚

symmint-pred : SymmInt -> SymmInt
symmint-pred (pos (p1 zero)) = szero
symmint-pred (pos (p1 (suc x))) = pos ( p1 x )
symmint-pred szero = neg ( p1 zero )
symmint-pred (neg (p1 x)) = neg (p1 (suc x))

symm-predsucc : ( n : SymmInt ) -> ( definition-equal ( symmint-pred ( symmint-succ n ) ) n )
symm-predsucc (pos (p1 zero)) = 🐓🥚
symm-predsucc (pos (p1 (suc x))) = 🐓🥚
symm-predsucc szero = 🐓🥚
symm-predsucc (neg (p1 zero)) = 🐓🥚
symm-predsucc (neg (p1 (suc x))) = 🐓🥚

symm-succpred : ( n : SymmInt ) -> ( definition-equal ( symmint-succ ( symmint-pred n ) ) n )
symm-succpred (pos (p1 zero)) = 🐓🥚
symm-succpred (pos (p1 (suc x))) = 🐓🥚
symm-succpred szero = 🐓🥚
symm-succpred (neg (p1 zero)) = 🐓🥚
symm-succpred (neg (p1 (suc x))) = 🐓🥚
...
symmint-pos-add-hom (p1 (suc x)) (p1 zero)
  rewrite ( r-add-zero {x} )
  rewrite ( r-add-one-suc {x} )
  = 🐓🥚
symmint-pos-add-hom (p1 (suc x)) (p1 (suc y))
...
symmint-mul-sign1 : ( p q : SymmInt )
   -> ( definition-equal
          ( symmint-mul ( symmint-mul symm-minusone p ) q )
          ( symmint-mul symm-minusone ( symmint-mul p q ) ) )
symmint-mul-sign1 p q
   rewrite ( symmint-mul-ass symm-minusone p q )
   = 🐓🥚

symmint-mul-sign2 : ( p q : SymmInt )
   -> ( definition-equal
          ( symmint-mul p ( symmint-mul symm-minusone q ) )
          ( symmint-mul symm-minusone ( symmint-mul p q ) ) )
symmint-mul-sign2 p q
   rewrite ( sym ( symmint-mul-ass p symm-minusone q ) )
   rewrite ( symmint-mul-comm p symm-minusone )
   rewrite ( symmint-mul-ass symm-minusone p q )
   = 🐓🥚

symmint-sign : SymmInt -> SymmInt
symmint-sign (pos x) = pos (p1 zero)
symmint-sign szero = szero
symmint-sign (neg x) = neg (p1 zero)

symmint-abs : SymmInt -> SymmInt
symmint-abs (pos x) = pos x
...
symmint-decomp (pos x)
   ...
   = 🐓🥚
symmint-decomp szero = 🐓🥚
symmint-decomp (neg x)
    rewrite ( pos-mul-lunital x )
    = 🐓🥚

symmint-mul-decomp : ( p q : SymmInt )
           -> ( definition-equal
                    ( symmint-mul
                        ( symmint-mul ( symmint-sign p ) ( symmint-sign q ) )
                        ( symmint-mul ( symmint-abs p ) ( symmint-abs q ) ) )
                    ( symmint-mul p q ) )
symmint-mul-decomp p q
    rewrite ( symmint-mul-ass (symmint-sign p) (symmint-sign q) (symmint-mul (symmint-abs p) (symmint-abs q)) )
    rewrite ( symmint-mul-comm (symmint-abs p ) ( symmint-abs q ) )
    rewrite ( sym ( symmint-mul-ass (symmint-sign q) (symmint-abs q) (symmint-abs p) ) )
    rewrite ( symmint-decomp q )
    rewrite ( symmint-mul-comm q ( symmint-abs p ) )
    rewrite ( sym ( symmint-mul-ass (symmint-sign p) (symmint-abs p ) q ) )
    rewrite ( symmint-decomp p )
    = 🐓🥚

symmint-abs-mul : ( p q : SymmInt )
            -> ( definition-equal
                    ( symmint-mul ( symmint-abs p ) ( symmint-abs q ) )
                    ( symmint-abs ( symmint-mul p q ) ) )
symmint-abs-mul (pos x) (pos x₁) = 🐓🥚
symmint-abs-mul (pos x) szero = 🐓🥚
symmint-abs-mul (pos x) (neg x₁) = 🐓🥚
symmint-abs-mul (neg x) (pos x₁) = 🐓🥚
symmint-abs-mul (neg x) szero = 🐓🥚
symmint-abs-mul (neg x) (neg x₁) = 🐓🥚
symmint-abs-mul szero q = 🐓🥚

symmint-sgn-mul : ( p q : SymmInt )
            -> ( definition-equal
                    ( symmint-mul ( symmint-sign p ) ( symmint-sign q ) )
                    ( symmint-sign (symmint-mul p q ) ) )
symmint-sgn-mul szero q = 🐓🥚
symmint-sgn-mul (pos x) (pos x₁) = 🐓🥚
symmint-sgn-mul (pos x) szero = 🐓🥚
symmint-sgn-mul (pos x) (neg x₁) = 🐓🥚
symmint-sgn-mul (neg x) (pos x₁) = 🐓🥚
symmint-sgn-mul (neg x) szero = 🐓🥚
symmint-sgn-mul (neg x) (neg x₁) = 🐓🥚
...
symmint-abs-ldist : ( l p q : SymmInt )
        -> ( definition-equal
                ( symmint-add ( symmint-mul ( symmint-abs l ) ( symmint-abs p ) )
                              ( symmint-mul ( symmint-abs l ) ( symmint-abs q ) ) )
                ( symmint-mul ( symmint-abs l ) ( symmint-add ( symmint-abs p ) ( symmint-abs q ) )  ) )
symmint-abs-ldist szero p q = 🐓🥚
symmint-abs-ldist p szero q
   rewrite ( symmint-mul-zero-absorbs ( symmint-abs p ) )
   rewrite ( add-int-comm (nat-int zero ) (symm-to-asymm-int (symmint-mul (symmint-abs p) (symmint-abs q))) )
   rewrite ( add-int-zero (symm-to-asymm-int (symmint-mul (symmint-abs p) (symmint-abs q))) )
   rewrite ( add-int-comm (nat-int zero ) (symm-to-asymm-int (symmint-abs q)) )
   rewrite ( add-int-zero (symm-to-asymm-int (symmint-abs q)) )
   rewrite ( assi (symmint-mul (symmint-abs p) (symmint-abs q)) )
   rewrite ( assi (symmint-abs q) )
   = 🐓🥚
symmint-abs-ldist p q szero
   rewrite ( symmint-mul-zero-absorbs ( symmint-abs p ) )
   rewrite ( add-int-zero (symm-to-asymm-int (symmint-mul (symmint-abs p) (symmint-abs q)) ) )
   rewrite ( add-int-zero (symm-to-asymm-int (symmint-abs q)) )
   rewrite ( assi ( symmint-abs q ) )
...
symmint-abs-ldist (pos x) (pos y) (pos z) 
   rewrite ( pos-add-hom (pos-mul x y) (pos-mul x z) ) 
   rewrite ( pos-add-hom y z ) 
   rewrite ( pos-mul-ldist x y z ) 
   = 🐓🥚
symmint-abs-ldist (pos x) (pos y) (neg z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (pos x) (neg y) (pos z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (pos x) (neg y) (neg z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (neg x) (pos y) (pos z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (neg x) (pos y) (neg z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (neg x) (neg y) (pos z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
symmint-abs-ldist (neg x) (neg y) (neg z) 
    rewrite ( pos-add-hom (pos-mul x y) ( pos-mul x z ) ) 
    rewrite ( pos-add-hom y z ) 
    rewrite ( pos-mul-ldist x y z ) 
    = 🐓🥚
